...
size = 'Medium'
use_early_link = True
early_link = 'http://shop.palaceskateboards.com/products/reversible-thinsulate-purple-teal'
use_keyword = False
#TODO: Make the logic for keyword checkout

#Functions
def checkout():
    response = session.get('http://shop.palaceskateboards.com/cart')
    ...
    response = session.post('http://shop.palaceskateboards.com/cart', data=payload)
    soup = bs(response.text, 'html.parser')

    form = soup.find('form', {'action' : re.compile('(?<=shop.palaceskateboards.com)(.*)(?=/checkouts/)')})
    ...
    payload = {
        ...
        'step' : 'payment_method',
        'checkout[shipping_rate][id]' : 'shopify-3-4%20Working%20Day%20Delivery-4.95',
        'button' : '',
        'checkout[client_details][browser_width]' : '123',
        'checkout[client_details][browser_height]' : '456',
        'checkout[client_details][javascript_enabled]' : '1'
    ...
    form = soup.find('form', {'data-payment-form' : re.compile('(?<=shop.palaceskateboards.com)(.*)(?=/checkouts/)')})
...
if use_early_link:
    ...
    response = session.post('http://shop.palaceskateboards.com/cart/add', data=payload)
